                ...
                with open(dfile) as df:
                    while True:
                        ...
                        self.input_ranking_vectors = np.zeros(
                            (self.batch_size, self.num_users, 5), dtype='int8')
                        self.output_ranking_vectors = np.zeros(
                            (self.batch_size, self.num_users, 5), dtype='int8')
                        self.input_mask_vectors = np.zeros(
                            (self.batch_size, self.num_users), dtype='int8')
                        ...
                        for i, line in enumerate(next_n_data_lines):
                            ...
                            if self.mode == 0:
                                ordering = np.random.permutation(
                                ...
                                for j, (user_id, value
                                        ) in enumerate(zip(user_ids, values)):
                                    if flag_in[j]:
                                ...
                                for j, (user_id, value, flag) in enumerate(
                                        zip(user_ids, values, flags)):
                                    if flag == 0:
                                        self.input_ranking_vectors[
                                            i, user_id, (value - 1)] = 1
                                    else:
